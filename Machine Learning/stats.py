...
def determine_method(filename, fselect):
    for m in fselect:
        if m in filename:
            return m

def summary_a_method(data, method, f_summary, fselect):   
    for c in data.columns:
        for f in data[c]:
            if f not in list(f_summary['feature']):
                d = {'feature':f}
                for i in range(len(fselect)):
                    d[fselect[i]]=0
                df = pd.DataFrame(d,index=[1])
                f_summary = pd.concat([f_summary,df], ignore_index = True)
            f_summary.loc[f_summary['feature']==f,method] = f_summary.loc[f_summary['feature']==f,method]+1
    return f_summary

def feature_summary(path, fselect, classifiers):
    os.chdir(path+'/features')
    f_summary = pd.DataFrame([], columns=['feature']+fselect)
    method_list = fselect.copy()
    for f in fselect:
        file = classifiers[0]+"_"+f+"_FEATURES.csv"
        method = determine_method(file, fselect)
        if method in method_list:
            method_list.remove(method)
            data = pd.read_csv(file).drop(['Unnamed: 0'],axis=1)
            f_summary = summary_a_method(data,method,f_summary, fselect)
...
def heatmap(path, title):
    ...
    for name in filenames:  
        ...
        if "cfs" in name:
            ...
            featureselection.append("CHI2-"+name.split('_')[1])
            group.append(2)
